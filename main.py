...
from utils.prediction.limb_prediction.models import load_limb_models
...
movement_detector = MovementDetector(movement_threshold=0.05, stability_threshold=10)
previous_moves = PreviousMoves(save_dir='/training_images/saved_moves')
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
detr_model_name = 'facebook/detr-resnet-50'
img_model_name = 'c14kevincardenas/ClimBEiT_single_frame'
climbeit_limb_model_name = 'c14kevincardenas/ClimBEiTv2'
hands_loc_model_name = 'c14kevincardenas/limbxy_hands'
feet_loc_model_name = 'c14kevincardenas/limbxy_feet'
print('Loading Moved Prediction Models...')
...
tts = TextToSpeech(rate=150, voice_index=1)
    ...
    while True:
        ...
        # run beta caller
            ...
            if state == "move_completed":
                ...
                limb, location = predict_move(frame, previous_moves, limb_models, location_models, device, use_seq)
            ...
            if draw_pose:
                frame = pose_frame
            if draw_holds:
                frame = draw_hold_bbs(frame, holds, target_hold)
        ...
        if key == ord('d'):
            ...
            if ipd_calc_mode:
                print("\nClimber, move to the wall and face the camera. 'd': back to finding holds. 'h': draw holds. "
